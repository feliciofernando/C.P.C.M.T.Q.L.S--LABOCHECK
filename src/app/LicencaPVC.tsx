...
export default function LicencaPVC() {
  ...
  const handleSearch = async () => {
    if (!search.trim()) {
      toast.error('Insira um numero de ordem, nome ou B.I.');
      return;
    }
    setLoading(true);
    setCondutor(null);
  ...
  const downloadPNG = async (type: 'front' | 'back') => {
    ...
    try {
      ...
      a.download = `Licenca_${type === 'front' ? 'Frente' : 'Tras'}_${nome}.png`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      toast.success(`Licença ${type === 'front' ? 'Frente' : 'Tras'} descarregada com sucesso (PNG)`);
    } catch {
  ...
  const downloadBoth = async () => {
    ...
    try {
      ...
      [frontBlob, backBlob].forEach((blob, i) => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `Licenca_${i === 0 ? 'Frente' : 'Tras'}_${nome}.png`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);
      ...
      toast.success('Licença Frente e Tras descarregadas com sucesso (PNG)');
    } catch {
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white border border-[#d1d1cc] rounded-lg p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-bold text-[#1a1a1a]">Criar Licenca</h3>
              <p className="text-xs text-[#6b6b6b]">
                Pesquise pelo Nome, B.I. ou No de Ordem do condutor
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder="Nome, B.I. ou No de Ordem..."
              className="flex-1"
            />
            <Button
            ...
            >
              ...
              {loading ? 'A pesquisar...' : 'Criar Licenca'}
            </Button>
          </div>
        </div>
      </div>
      ...
      {!loading && condutor && (
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-xs text-[#6b6b6b] ml-3">
                  No {condutor.numeroOrdem} | {condutor.status as string}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start justify-center">
            ...
            {/* BACK */}
            <div className="text-center">
              <p className="text-sm font-bold text-[#6b6b6b] mb-3">TRAS</p>
              {backImgUrl && (
                <img
                  src={backImgUrl}
                  alt="Licença Tras"
        ...
        <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-[#1a5c2e] flex items-center gap-2">
              ...
              Licenca Profissional de Condutor (PNG)
            </DialogTitle>
          </DialogHeader>
          {condutor && (
            <div className="space-y-8 py-4">
              ...
              <div className="text-center">
                <p className="text-lg font-bold text-[#1a1a1a] mb-3">TRAS</p>
                <img
                  src={backImgUrl}
                  alt="Licença Tras"
                  className="max-w-full mx-auto rounded-lg shadow-lg border border-[#d1d1cc]"
