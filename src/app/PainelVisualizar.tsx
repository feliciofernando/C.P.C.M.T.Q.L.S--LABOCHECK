...
export default function PainelVisualizar() {
  ...
  const handleDownloadFicha = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Ficha_Registo_${"00" + condutorDetail.numeroOrdem}_${String(condutorDetail.nomeCompleto).replace(/\s+/g, '_')}.pdf`;
  ...
  const handleDownloadQR = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `QR_Code_${"00" + condutorDetail.numeroOrdem}_${String(condutorDetail.nomeCompleto).replace(/\s+/g, '_')}.jpg`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
              ...
              {loading ? (
                ...
                condutores.map((c) => (
                  <TableRow key={c.id} className="hover:bg-[#f8f8f5]">
                    <TableCell className="font-medium font-mono">{"00" + c.numeroOrdem}</TableCell>
                    <TableCell>
                      <div>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-[#1a5c2e]">
              Ficha de Registo - Nº {"00" + selectedCondutor?.numeroOrdem}
            </DialogTitle>
          </DialogHeader>
          {loadingDetail ? (
