  ...
  try {
    ...
    for (const condutor of condutores || []) {
      ...
      if (diffDias < 0) {
        ...
        mensagem = `A Licença Profissional de Condutor de ${condutor.nome_completo} (Nº ${condutor.numero_ordem}) expirou há ${diasExpirada} dia${diasExpirada !== 1 ? 's' : ''}. Data de validade: ${condutor.validade_licenca}. Contacto: ${condutor.telefone1}.`;
      } else if (diffDias <= 30) {
        tipo = 'EXPIRANDO_1MES';
        prioridade = diffDias <= 7 ? 'ALTA' : 'MEDIA';
        mensagem = `A Licença Profissional de Condutor de ${condutor.nome_completo} (Nº ${condutor.numero_ordem}) expira em ${diffDias} dia${diffDias !== 1 ? 's' : ''}. Data de validade: ${condutor.validade_licenca}. Contacto: ${condutor.telefone1}.`;
      } else {
        continue;
      }
    ...
    return NextResponse.json({
      message: `Verificação concluída. ${alertasCriados.length} alerta(s) processado(s).`,
