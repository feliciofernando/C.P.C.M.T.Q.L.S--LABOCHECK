...
// No accented characters, no HTML entities - maximum compatibility
...
function buildFichaSVG(c: Record<string, unknown>): string {
  ...
  const sectionHeader = (num: string, title: string): string => {
    let out = '';
    out += `<text x="${ML}" y="${y}" fill="${GREEN}" font-size="13" font-weight="bold" font-family="${FF}">${num}. ${title}</text>\n`;
    addY(5);
    out += `<line x1="${ML}" y1="${y}" x2="${MR}" y2="${y}" stroke="${DGRAY}" stroke-width="0.5"/>\n`;
    addY(9);
    return out;
  };
  ...
  const docs: [string, string][] = [
    ...
    ['temCapacete', 'Capacete de Proteccao'],
