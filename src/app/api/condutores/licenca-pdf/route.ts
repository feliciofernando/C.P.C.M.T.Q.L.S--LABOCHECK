...
// Simple XML escape - only handles the 5 XML-special characters
...
function buildFrontSVG(c: Record<string, unknown>): string {
  ...
  svg += `<text x="${W / 2}" y="56" text-anchor="middle" fill="${GOLD}" font-size="14" font-weight="bold" font-family="${FF}" letter-spacing="1">LICENCA PROFISSIONAL DE CONDUTOR</text>`;
  ...
  const leftField = (label: string, value: string) => {
    let out = `<text x="${LX}" y="${ly}" fill="${GREEN}" font-size="8" font-weight="bold" font-family="${FF}">${esc(label)}:</text>`;
    out += `<text x="${LX}" y="${ly + 13}" fill="${DARK}" font-size="11" font-weight="bold" font-family="${FF}">${esc(value || '-')}</text>`;
    return out;
  };

  const rightField = (label: string, value: string) => {
    let out = `<text x="${RX}" y="${ry}" fill="${GREEN}" font-size="8" font-weight="bold" font-family="${FF}">${esc(label)}:</text>`;
    out += `<text x="${RX}" y="${ry + 13}" fill="${DARK}" font-size="11" font-weight="bold" font-family="${FF}">${esc(value || '-')}</text>`;
    return out;
  };
...
function buildBackSVG(c: Record<string, unknown>): string {
  ...
  svg += `<text x="30" y="24" fill="${GREEN}" font-size="10" font-weight="bold" font-family="${FF}">Data de Emissao: ${esc(String(c.dataEmissaoLicenca || '-'))}</text>`;
  ...
  svg += `<text x="${W / 2}" y="${ly + 22}" text-anchor="middle" fill="${WHITE}" font-size="8" font-family="${FF}">Decreto Presidencial No 245/15</text>`;
  ...
  // Motto
  svg += `<text x="${W / 2}" y="${H - 38}" text-anchor="middle" fill="${DARK}" font-size="9" font-family="${FF}">"Mototaxistas organizados, transito mais seguro"</text>`;
